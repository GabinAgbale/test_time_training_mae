...
def train_on_test(base_model: torch.nn.Module,
    ...
    eval_loader = list(torch.utils.data.DataLoader(dataset_eval, batch_size=1, shuffle=False, num_workers=args.num_workers))
    # Save classes
    classes = np.zeros(dataset_len)
    ...
    for data_iter_step in range(iter_start, dataset_len):
        ...
        classes[data_iter_step] = test_label
        pseudo_labels = None
        # Test time training:
        ...
        if data_iter_step % 100 == 1:
            acc_step = 0
            with torch.no_grad():
                model.eval()
                for i in range(1000):
                    (eval_samples, eval_labels) = eval_loader[i]
    ...
    # Save class labels
    with open(os.path.join(args.output_dir, 'classes.npy'), 'wb') as f:
                np.save(f, classes)
